...
class VLLMServerWidget(QtWidgets.QGroupBox):
    ...
    # Remote-mode cross-thread signals (emitted from worker threads, delivered on Qt main thread).
    _remote_gpu_updated   = pyqtSignal(float, float)        # (free_gb, total_gb)
    _remote_vllm_event    = pyqtSignal(int, str, str, str)  # (server_id, status, model_id, error)
    ...
    def __init__(
        ...
        # Remote-mode state: populated when IS_REMOTE_MODE=True.
        self._remote_gpu_free_gb: float = 0.0
        self._remote_gpu_total_gb: float = 0.0
        self._remote_channel: Any = None
        self._remote_inference_stub: Any = None
        self._remote_system_stub: Any = None
        self._remote_executor: Optional[ThreadPoolExecutor] = None
        self._remote_watch_stop: Optional[threading.Event] = None
        self._remote_watch_thread: Optional[threading.Thread] = None
        self._system_info_timer: Optional[QTimer] = None

        if IS_REMOTE_MODE:
            self._setup_remote_stubs()

        self._build_ui()

    def _get_total_gpu_memory(self) -> float:
        """Get total GPU memory in GB — returns cached remote value in remote mode."""
        if IS_REMOTE_MODE:
            return self._remote_gpu_total_gb
        try:
            import pynvml
            pynvml.nvmlInit()
    ...
    def _get_gpu_free_memory(self) -> float:
        """Get free GPU memory in GB — returns cached remote value in remote mode."""
        if IS_REMOTE_MODE:
            return self._remote_gpu_free_gb
        try:
            import pynvml
            pynvml.nvmlInit()
            handle = pynvml.nvmlDeviceGetHandleByIndex(0)
    ...
    def _start_server(self, server_id: int) -> None:
        ...
        if IS_REMOTE_MODE:
            # Remote daemon: ask InferenceService to start vLLM on the server.
            # Status updates arrive via the WatchVLLMServers stream (_on_remote_vllm_event).
            state.status = "starting"
            self._update_row_state(server_id, state)
            gpu_util = get_settings().vllm_gpu_memory_utilization / max(1, self._current_server_count)
            self._start_server_remote(server_id, model_path, gpu_util)
            return
    ...
    def _stop_server(self, server_id: int) -> None:
        ...
        """
        if IS_REMOTE_MODE:
            # Ask InferenceService to terminate vLLM on the server.
            # Optimistically mark stopped locally; WatchVLLMServers confirms.
            self._stop_server_remote(server_id)
    ...
    # ------------------------------------------------------------------
    # Remote-mode helpers (IS_REMOTE_MODE=True only)
    # ------------------------------------------------------------------

    def _setup_remote_stubs(self) -> None:
        """Create synchronous gRPC channel and stubs for remote-mode calls.

        Uses synchronous (non-aio) stubs so unary calls can be made from a
        ThreadPoolExecutor without needing a separate asyncio event loop.
        ...
        self._remote_channel = _grpc.insecure_channel(DAEMON_TARGET)
        self._remote_inference_stub = inference_pb2_grpc.InferenceServiceStub(self._remote_channel)
        self._remote_system_stub   = system_pb2_grpc.SystemServiceStub(self._remote_channel)
        self._remote_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="vllm-remote")

        # Connect cross-thread signals to Qt-main-thread slots.
        self._remote_gpu_updated.connect(self._on_remote_gpu_updated)
        self._remote_vllm_event.connect(self._on_remote_vllm_event)

        # System info poll every 5 s.
        self._system_info_timer = QTimer(self)
        self._system_info_timer.timeout.connect(self._poll_remote_system_info)
        self._system_info_timer.start(5000)
        self._poll_remote_system_info()  # immediate first poll

        # WatchVLLMServers stream in a daemon thread.
        self._remote_watch_stop = threading.Event()
        self._remote_watch_thread = threading.Thread(
            target=self._watch_vllm_loop,
            daemon=True,
            name="vllm-watch",
        )
        self._remote_watch_thread.start()

    def _poll_remote_system_info(self) -> None:
        """Fetch GPU info from remote SystemService (runs in thread pool)."""
        from gym_gui.services.trainer.proto.system_pb2 import SystemInfoRequest
        stub = self._remote_system_stub
        if stub is None:
            return

        def _fetch():
            try:
                return stub.GetSystemInfo(SystemInfoRequest(), timeout=4)
            except Exception:
                return None

        def _done(fut):
            resp = fut.result()
            if resp and resp.gpus:
                gpu = resp.gpus[0]  # primary GPU
                free_gb  = gpu.memory_total_gb - gpu.memory_used_gb
                total_gb = gpu.memory_total_gb
                self._remote_gpu_updated.emit(free_gb, total_gb)

        if self._remote_executor:
            self._remote_executor.submit(_fetch).add_done_callback(_done)

    def _watch_vllm_loop(self) -> None:
        """Background daemon thread: subscribe to WatchVLLMServers and emit Qt signals."""
        from gym_gui.services.trainer.proto.inference_pb2 import WatchVLLMRequest
        stub = self._remote_inference_stub
        stop = self._remote_watch_stop
        if stub is None or stop is None:
            return
        try:
            stream = stub.WatchVLLMServers(WatchVLLMRequest())
            for event in stream:
                if stop.is_set():
                    break
                self._remote_vllm_event.emit(
                    event.server_id,
                    event.status,
                    event.model_id,
                    event.error_message,
            ...
            _LOGGER.debug("WatchVLLMServers stream ended: %s", exc)

    def _start_server_remote(self, server_id: int, model_path: str, gpu_util: float) -> None:
        """Submit StartVLLMServer gRPC call on the thread pool (non-blocking)."""
        from gym_gui.services.trainer.proto.inference_pb2 import StartVLLMRequest
        # Widget uses 1-based server IDs; proto uses 0-based (0 -> port 8000).
        ...
        def _call():
            try:
                if stub:
                    stub.StartVLLMServer(req, timeout=10)
            except Exception as exc:
                # Emit an error event so the UI updates correctly.
                self._remote_vllm_event.emit(server_id, "error", "", str(exc))

        if self._remote_executor:
            self._remote_executor.submit(_call)

    def _stop_server_remote(self, server_id: int) -> None:
        """Submit StopVLLMServer gRPC call on the thread pool (non-blocking)."""
        from gym_gui.services.trainer.proto.inference_pb2 import StopVLLMRequest
        req = StopVLLMRequest(server_id=server_id - 1)
        stub = self._remote_inference_stub

        def _call():
            try:
                if stub:
                    stub.StopVLLMServer(req, timeout=10)
            except Exception as exc:
                _LOGGER.debug("StopVLLMServer error: %s", exc)

        if self._remote_executor:
            self._remote_executor.submit(_call)

    def _on_remote_gpu_updated(self, free_gb: float, total_gb: float) -> None:
        """Slot: update cached remote GPU memory and refresh the header label."""
        self._remote_gpu_free_gb  = free_gb
        self._remote_gpu_total_gb = total_gb
        self._update_info_label()

    def _on_remote_vllm_event(self, server_id: int, status: str,
                               model_id: str, error: str) -> None:
        """Slot: apply a VLLMServerEvent from the WatchVLLMServers stream to the UI.

        The proto uses 0-based server_id; the widget uses 1-based — convert back.
        """
        widget_server_id = server_id + 1
        state = self._server_states.get(widget_server_id)
        if state is None:
            return
        state.status = status
        state.error_message = error or None
        if status == "stopped":
            state.memory_gb = 0.0
        self._update_row_state(widget_server_id, state)
        self.server_status_changed.emit(widget_server_id, status, model_id)
        self._update_info_label()
    ...
    def cleanup(self) -> None:
        ...
        self._health_timer.stop()
        if IS_REMOTE_MODE:
            # Signal the WatchVLLMServers thread to exit and close the channel.
            if self._remote_watch_stop:
                self._remote_watch_stop.set()
            if self._system_info_timer:
                self._system_info_timer.stop()
            if self._remote_executor:
